...
You must ensure that the public Ethereum address generated above has at least **0.2 ETH** on Ethereum Mainnet and **0.1 ETH** on Ethereum Arbitrum. At the end of every epoch, an attempt to refund the validator fees occurs. Only if the total refunded amount is 100x the fee it would cost to do the refund does the refund actually occur. You should monitor this balance to ensure it doesn't fall below the required amount.
