...
def priceToTick(price, decimals_of_quote_asset, decimals_of_base_asset):
...
def tickToPrice(tick, decimals_of_quote_asset, decimals_of_base_asset):
...
```py
def numberToFixed(number, fractional_precision):
    return round(number * pow(2, fractional_precision))

def fixedToNumber(fixed_precision_number, fractional_precision):
...
```py
def priceToHexPrice(price, decimals_of_quote_asset, decimals_of_base_asset):
    shifted = round(price * pow(2, 128))
    return "0x{:02x}".format(shifted * pow(10, decimals_of_quote_asset) // pow(10, decimals_of_base_asset))

def hexPriceToPrice(hex_price, decimals_of_quote_asset, decimals_of_base_asset):
...
```py
def sqrtPriceToPrice(hex_input, decimals_of_quote_asset, decimals_of_base_asset):
    raw_price = pow(int(hex_input, 16)/pow(2,96),2);
    return raw_price * pow(10, decimals_of_base_asset - decimals_of_quote_asset);

def priceToSqrtPrice(price, decimals_of_quote_asset, decimals_of_base_asset):
...
Parameters:
- [`asset_amount`](#amount): The amount to be transfered.
- [`asset`](#asset): The asset to be transfered.
