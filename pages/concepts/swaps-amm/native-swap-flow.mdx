...
<Callout type="info">
  Funds sent straight to Chainflip vaults without initiating it using one of
  these methods **will result in the loss of funds**, as the network won't know
  what to do with it.
</Callout>
...
<Callout type="info">
  Pre-witnessing will allow the network to confirm transactions faster or slower
  depending on the value of that deposit. This will cut down the confirmation
  window for small deposits, and increase the security for larger transfers.
