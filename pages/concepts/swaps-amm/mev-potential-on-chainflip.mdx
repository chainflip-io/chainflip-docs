...
In Uniswapv3, a slippage limit exists such that if a swap drops below a user-specified rate, it is aborted. 
This feature is also implemented in the JIT AMM, and Fill-or-Kill has been a mandatory feature since 1.8, which retries and refunds users if swaps do not satisfy a user defined [minimum price or oracle price slippage](/swapping/swapping-basics#slippage-protection). 
