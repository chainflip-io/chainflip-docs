...
```
This event is emitted by the chain when nodes are being reported for failing to witness in time. You can trace the call that had failed to be witnessed by its `CallHash`, and the block number at which the call was successfully witnessed and dispatched . If you then go to the target block, you can then look for this event:
...
This event is emitted together with the call that has just been successfully witnessed, (look for events and extrinsics around this event). You can get more information about what the call was and gain useful insight (such as call type, the external chain etc). 
