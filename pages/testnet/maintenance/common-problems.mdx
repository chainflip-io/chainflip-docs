...
sudo systemctl status chainflip-engine1.1
```
...
If your node fails to broadcast a transaction there can be multipe reasons for that:
- Your validator is out of gETH
- Your connection or the connection to the endpoints is unstable

This can happen from time to time (due to temporary connection problems) and it results only in a loss of reputation, but if this keeps happening then be sure to top up you gETH and check the health of your endpoints.
...
```bash copy
sudo systemctl restart chainflip-engine1.1.service
```
