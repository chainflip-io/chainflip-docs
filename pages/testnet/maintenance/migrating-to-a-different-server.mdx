...
```bash copy
sudo apt-get update
sudo apt-get install -y chainflip-cli chainflip-node chainflip-engine1.2
```
...
sudo systemctl disable --now chainflip-engine1.2.service
sudo apt purge chainflip-node
sudo apt purge chainflip-engine1.2
```
