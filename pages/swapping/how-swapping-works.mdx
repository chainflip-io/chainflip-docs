...
1. **Initiating a swap:** This first step can be done through the [Swapping app](https://swap.chainflip.io/) (frontend), the [Javascript SDK](integrations/javascript-sdk/introduction), or directly using the [Chainflip CLI client](integrations/running-a-broker/broker-api#broker_request_swap_deposit_address). There are two primary methods to start a Chainflip Swap:
   ...
   - **Direct call to the Chainflip Vault:** This method involves including a particular payload in a deposit transaction. It will be a function call to a smart contract for EVM chains and a program call to a program for Solana. This facilitattes integration for more advanced users (like DEX Aggregators). For more details see [Vault swaps](integrations/advanced/vault-swaps.mdx).
