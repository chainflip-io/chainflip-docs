...
3. The protocol attempts to [*Boost* the incoming deposit](../lp/boost/how-boost-works#boosting-a-deposit), by checking the available *Boost* liquidity provided by LPs
4. If there is enough collateral in the *Boost* pools, the protocol pre-witness the deposit **after it has been included in a block**, minus a small *Boost* fee
5. As soon as the [deposit is witnessed](../concepts/validator-network/ingress-witnessing-deposits#witnessing), the swap is triggered by the [JIT AMM](../concepts/swaps-amm/just-in-time-amm-protocol), and [sends the resulting ETH](../concepts/validator-network/egress-broadcasting-funds) to the destination address
...
- The user's intended amount acts as an upper limit; anything less increases the chances of a successful boost
