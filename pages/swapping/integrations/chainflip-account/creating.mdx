...
title: Generating Chainflip Account Keys

description: In order to create a chainflip account, you will need a Chainflip-native key pair for interacting with the State Chain.
...
Brokers nodes perform actions on the Chainflip network, and all of these actions are authorised using cryptographic keys. Specifically, to run a broker, you will need a Chainflip-native key pair for interacting with the State Chain.
...
<Callout type="info">
We recommend generating keys on the machine then using GPG encryption to securely transfer the ciphertext to a backup location. This is to eliminate the risk of the keys being intercepted in transit or being cached in the clipboard in the case of copying them from the terminal.
</Callout>
...
Take special note of the **Validator Account ID** beginning with `cF`. This is your Broker ID.
