...
2. This can be done by manually encoding the transaction (e.g. making the smart contract call) or by using the `request_swap_parameter_encoding` RPC call from our Broker Api

3. If using the Broker API, an encoded payload is obtained and must be signed and sent to the appropriate blockchain.

4. The transaction will tranfer the appropriate funds to the Chainflip protocol and right the smart contract call. Chainflip will then process the swap.

<Callout type="warning">

It is possible to manually call the Vault smart contract without using the provided RPCs to encode the transaction. 
The smart contract does some checks bit it cannot check that all the parameters are valid and correctly encoded.
Invalid or wrongly encoded parameters will result in a failure to initiate a swap. 

**This may result in the loss of user funds.**

We strongly advice using the RPC provided to craft the Vault Swap transactions. Otherwise make sure to thoroughly test your logic before initiating a swap with real funds.
...
// TODO: Put the tables side to side.
...
<Callout type="info">
This section is only relevant if you want to encode the transaction manually instead of using the provided RPC helpers.
...
The `cf_parameters` is one of the parameter required as part of EVM and Solana Vault Swap calls when manually encoding the transaction to initiate the Vault swap.

As described in the [EVM smart contract interface](./evm.mdx#initiate-simple-swap) and  the [Solana program interface](./solana.mdx#parameter-reference-for-the-program-call), the `cf_parameters` must be passed as part of the transaction data. The protocol expects a `VersionedCfParameters` encoded into bytes format using Parity's Scale.
...
pub struct VaultSwapParameters {
  ...
  // DCA paramters. See Vault Swaps page for its type reference.
...
// TODO: Add example of how to encode the `cf_parameters` manually in typescript. Check out legacy code in the SDK.