...
As described so far, initiating a swap involves requesting a deposit address and sending funds to that deposit address to trigger the swap. Here we introduce an alternative way to initiate swaps on Chainflip by making a direct deposit to the Chainflip Vault. Vault Swaps support the same features as those initiated via [Deposit Channels](/concepts/swaps-amm/deposit-channels-and-brokers.mdx#deposit-channels).

The advantage of a vault swap is that the end-user can be presented with a transaction almost immediately, whereas creation of a deposit channel requires submitting a transaction to the Chainflip network, and waiting for inclusion in a block.

The downside of a vault swap is that the transaction is more complex and will incur higher fees compared to making a simple transfer to a deposit channel.
...
![Workflow for Swapping via Deposit Channel](./deposit_channel.jpeg)
...
2. The generated Deposit address is returned as a response from the RPC call.

3. Deposit fund into the given deposit address.

The remaining steps are shared between both swap workflows.

4. Swaps are processed after deposits are witnessed by Chainflip Engine.

5. The swaps are scheduled and processed within the Chainflip Protocol's internal AMM system. 

6. The swap output is egressed either into the destination address (simple swaps) or the Smart Contract call for CCM swaps.

### Vault swaps

![Workflow for Swapping via Vault swaps](./vault_swaps.jpeg)
...
* This can be done by manually encoding the transaction (e.g. making the smart contract call) or by using the `request_swap_parameter_encoding` RPC call from our Broker API. It's encouraged to use the Broker API to ensure that the payloads are correctly encoded according to the specifications. Wrongly encoded payloads may result in loss of funds.

2. If using the Broker API, an encoded payload is obtained and must be signed and sent to the appropriate blockchain.

3. The transaction will transfer the appropriate funds to the Chainflip protocol as part of the smart contract call. Chainflip will then process the swap.

The remaining steps are identical to swapping via Deposit Channels.

### Pros and Cons

{ 
//TODO: Review this by Albert 
}

The two swap methods each have their own pro and cons. You can use the following information to choose the methods that best suits your situation.

| Considerations | Deposit Channels | Vault swaps |
| -------------- | ---------------- | ----------- | 
| No. of extrinsic calls   | 2                | 1           |
| Time it takes to start a swap | > 6 sec | < 1 sec |
| Repeated swaps for the same user | Supported | unsupported |
| Bitcoin CCM support | Yes         | No          |


## Constructing a Vault Swap Transaction

Each of our supported chains encodes data differently. To make this feature easy to use, Chainflip provides the interface to build the transaction on the integrator's behalf. This way the transaction only needs to be signed and is then ready to be sent out to the respective Vault contract.
...
You may see the use of the `cf_get_vault_swap_details()` RPC call in the documentation (e.g. in example JS code). This call is exactly the same (they share the same underlying logic) as `broker_request_swap_parameter_encoding()` but with the `broker_id` defined explicitly. Within this documentation, you will see the two being used interchangeably. In practice, you can use the RPC call that suits your situation better.
...
<Callout type="info">
Currently only Solana and EVM chains supports CCM. This means **CCM is only allowed with vault swaps with `output asset` being on the Solana or EVM chain (such as Sol or ArbEth)**. The mechanism differs between Solana and EVM chains, please refer to [CCM support](../cross-chain-messaging/cross-chain-messaging.mdx) for more details.
</Callout>
...
With the payload encoded, the next step is to construct a transaction from that payload and sent it to the correct chain's Vault contract address. This step differs for each chain/asset. Please refer to the page dedicated for each chain for more details and other information.

<Callout type="info">
The chain you need to interact with depends on the `Input/Source asset`. 
e.g. if you are swapping ETH into BTC, you should check out the page dedicated to Ethereum and Arbitrum.
</Callout>
...
Payloads and therefore transactions for Vault swaps are valid for long periods of time and in some cases indefinitely. However, it is strongly recommended to only sign and broadcast freshly encode payloads and to never rely on previous unused payloads.
