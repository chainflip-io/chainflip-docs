...
Each of our supported chain encodes data differently. To make this feature easy to use, Chainflip provides the interface to build the transaction on the integrator's behalf. This way the transaction only needs to be signed and is then ready to be sent out to the perspective Vault contract.

The function to that builds the call is `request_swap_parameter_encoding()` RPC call from the Broker Api. 
...
| extra_parameters | Chain specific extra parameters. Depends on the `source_asset`. For details see each chain's dedicated page | `VaultSwapExtraParametersRpc` |
...
<Callout type="info">
Each chain works a differently. Notice here for Solana, for example, the `input_amount` is required. This is not the case for some of other chains (such as Ethereum). Please refer to the individual chain's page for more details.
</Callout>
