...
<Callout type="info">
This section is only relevant if you want to build the transaction manually.
...
The `cf_parameter` is one of the parameter required as part of the Vault Swap call. It expects a `VersionedCfParameters` encoded into bytes format using Parity's Scale.
