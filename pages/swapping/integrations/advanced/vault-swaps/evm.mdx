...
These three fields map closely to common EVM transaction building APIs and need to be included in the transaction. Other values such as `gas` or `gasPrice` can be estimated and set by the user or wallet.
...
const encodedVaultSwapDetails = await response.json() as EvmVaultSwapDetails;
