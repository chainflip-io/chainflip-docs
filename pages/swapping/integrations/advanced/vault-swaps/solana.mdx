...
The built Solana Instruction are returned as the response. The next step is to sign and send this Vault Swap transaction to our Swap endpoint smart program.
...
| ccm_parameters | Additional metadata for swaps with Cross-Chain Messaging. Should be set to `None` for simple swaps | `Option<(message, gasAmount)>` |
...
| cfParameters | Additional metadata for additional features. See below. | `Vec<u8>` |
...
### Cross-chain-messages CCM

Vault swaps supports CCM messages. Currently CCM is only supported on the Ethereum/Arbitrum and Solana chain. This section provides information for CCM message attached as part of the Vault Swap call. This section provides information for when the `Output asset` is `Sol` or `SolUsdc`, and the Vault swap contains CCM.

<Callout type="info">
For more information on CCM in general see [CCM](../cross-chain-messaging/cross-chain-messaging.mdx), and for the Solana chain, see [CCM for Solana](../cross-chain-messaging/sol-ccm.mdx).
</Callout>

To send CCM as part of the Vault Swap call, simply fill out the corresponding fields and the call will be encoded automatically.

TODO: example of CCM. Write about CCM validity checks.
```sh
curl -H "Content-Type: application/json" -d '{
    "id": 1,
    "jsonrpc": "2.0",
...
#### cf_parameters
TODO: give encoding spec for cf_parameters
