...
// TODO: Consider trying and changing (or explaining) how `sendAndConfirmTransaction` could be replaced for transaction.sign() for newEventAccountKeypair and then transaction.sign() for userkeypair. Then sending it.
// This is what integrators will really do since they probably can't sign on top after the user has signed. In that case, according the the link below, we might need to set the `tx.feePayer` to the user keypair before signing with the event account Keypair.
// All the above needs to be tried before to make sure it works.
