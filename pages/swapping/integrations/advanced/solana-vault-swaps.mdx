...
For the details on how to call the RPC endpoint to get the instruction for the Vault swap, please refer to the [TODO: Link to RPC docs](TODO). The `cf_get_vault_swap_details` RPC call returns the instruction that will initiate a Vault swap. As per a regular Solana instruction (e.g. see [web3.js Instruction](https://solana-labs.github.io/solana-web3.js/classes/TransactionInstruction.html)), it contains:
