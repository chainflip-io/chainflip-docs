...
// Example estimating the call to the call to `cfReceive` with the CCM message, srcChain... Using ABI encoded data here for simplicity.
...
<Callout type="info">
Arbitrum L1 inclusion fees depend on the l1 gas price so it fluctuates over time. Therefore the gas budget required for an Arbitrum CCM call can also vary over time. It is recommended to run new gas estimations especially when there are large gas price fluctuations on the L1 or to manually increase the gas budget to account for that.
</Callout>
