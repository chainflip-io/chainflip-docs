...
## Smart contract interface - Receive call and asset on the receiver contract (CCM)
...
For Ethereum, the `ccm_additional_data` is not used by the Smart Contract call. When a Swap is requested into the EVM chain with CCM messages attached, if the ``ccm_additional_data` is **not** empty, the swap request will be rejected.

See [here](./additional-data.mdx#for-evm) for more info.