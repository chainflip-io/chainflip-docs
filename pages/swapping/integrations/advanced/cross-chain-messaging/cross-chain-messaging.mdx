...
As described in [How Swapping Works](../../../how-swapping-works.md), swaps in Chainflip can be initiated via requesting a deposit address or via a smart contract call (only in EVM chains). Both of those methods support cross-chain messaging by specifying some extra data.
...
<Callout type="warning">
  This docs are for post 1.8 release. There has been some breaking changes related to cross-chain messaging.
  Previously the `gas_budget` had a different meaning, check the . Also the `ccm_additional_data` was instead named `cf_parameters` and was unused.
  Make sure you know which is the current version of the protocol.
</Callout>
...
For deposit channels the `ccm_additional_data` is also required but it's only relevant for CCM swaps to Solana as explained in the [Solana CCM](./sol-ccm##solana-ccm-additional-data) section.
