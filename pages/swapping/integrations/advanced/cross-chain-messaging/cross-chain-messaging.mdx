...
The additional data required for a CCM swap, the `channel_metadata`, is shown below. It must be passsed when requesting a deposit channel via `request_swap_deposit_address` or when encoding a Vault swap via `request_swap_parameter_encoding()` RPC call.
