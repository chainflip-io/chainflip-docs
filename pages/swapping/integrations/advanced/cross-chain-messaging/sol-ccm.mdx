...
A CCM transaction can revert for multiple reasons, including but not limited to having insufficient compute units or reverting user logic on the receiver program. In that scenario assets won't be able to be egressed to the intended address.

Unlike EVM chains, Solana does not support try-catch mechanism. The Fallback mechanism is there to ensure funds are safely egressed to a user-provided fallback address in case of a CCM transaction reverting.
