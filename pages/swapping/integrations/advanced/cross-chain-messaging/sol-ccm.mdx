...
| `remaining_accounts` | Destination [asset ID](./vault-swaps.mdx#supported-assets-and-chains). | Pubkey[] |
...
The addresses included in the `ccm_additional_data` must not contain addresses specially reserved for Chainflip's use - that is primarily the current aggregate key and our Vault accounts. If such blacklisted accounts are contained, the RPC call to request a deposit address or encode a Vault Swap will fail.

</Callout>

If the list is invalid or the encoding is incorrect the opening of a deposit channel or the Vault Swap will fail.

The following Typescript code contains logic you can use to encode accounts into the correct encoding to build CCM for the Solana chain. You will need to add `@polkadot/util`, `@chainflip/cli`, `@chainflip/utils` and `scale-ts` as dependencies into your `package.json` file.
