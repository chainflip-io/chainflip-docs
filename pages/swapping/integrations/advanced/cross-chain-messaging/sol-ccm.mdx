...
To accommodate for that, a list of accounts needs to be passed as `ccm_additional_data` when opening a deposit channel or encoded into the the `cf_parameters` ({`Vec<u8>`}) parameter for CCM Vault swaps. That list must contain:
