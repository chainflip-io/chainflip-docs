...
3. **Swap assets:** There are two main ways to swap assets, depending on the source chain and/or the specific use case:
   1. **Request a [regular](/swapping/integrations/javascript-sdk/swap-assets/request-deposit-address/v2#regular-deposit-address) or [boostable](/swapping/integrations/javascript-sdk/swap-assets/request-deposit-address/v2#boostable-deposit-address) deposit address:** The `requestDepositAddress` method provides a deposit address where the user can send the source tokens to be swapped. Funds need to be sent to initiate the swap. The swapped tokens will be sent to the destination address provided.
