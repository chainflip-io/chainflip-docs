  ...
  <tbody>
    ...
    <tr>
      ...
      <td>
        ...
        It is recommended to understand under which conditions <a href="/integration/boost/how-boost-works/boosting-a-deposit" target="_blank">deposits get boosted</a>.
      </td>
      <td><code>number</code></td>
    </tr>
    <tr>
      <td>
        <code>refundParameters</code><span class="param-optional-label">(optional)</span>
      </td>
      <td>
        Optional metadata for setting a minimum accepted price for swaps through the deposit channel. This allows to
...
## refundParameters: Slippage protection

The optional `refundParameters` object sets a minimum accepted price for swaps triggered through the deposit channel.
...
If the minimum price cannot be met with the available liquidity int he specified amount of time, deposited assets will
be refunded to the specified address.
...
{<table>
  ...
  <tbody>
    <tr>
      ...
      <td>
        Minimum accepted price for swap triggered through the deposit channel. A price is a fixed-point number with
        a 128 bit fractional part (Q128.128) that represents the exchange rate between the base units of two assets.
    ...
    <tr>
      ...
      <td>
        Number of State Chain blocks to retry the swap with the minimum price before the deposit is refunded.
...
const swapDepositAddressRequest = {
  ...
  brokerCommissionBps: 100, // 100 basis point = 1%
  refundParameters: {
    minPrice: quote.estimatedPrice * 99 / 100, // minimum accepted price for swaps through the channel
    refundAddress: '0xa56A6be23b6Cf39D9448FF6e897C29c41c8fbDFF', // address to which refunds are sent
...
## ccmMetadata: Cross-Chain Messaging (CCM)
