...
`dcaParams` must also be set in conjunction with `fillOrKillParams`. This causes the `minPrice` to apply to each individual chunk in the overall swap. If a chunk fails to meet the `minPrice`, it will retry according to `retryDurationBlocks` until it does. If it does not meet it, the entire swap is retired. The swapped amount is egresed and anything that was not swapped is refunded back to the `refundAddress`
