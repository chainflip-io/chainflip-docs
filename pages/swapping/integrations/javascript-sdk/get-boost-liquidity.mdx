...
```typescript copy
getBoostLiquidity({ chainAsset?: UncheckedAssetAndChain, feeTierBps?: number }): Promise<BoostPoolDepth[]>
```

{
  <table>
   ...
   <tbody>
      <tr>
         <td><code>chainAsset</code><span class="param-optional-label">(optional)</span></td>
         <td>Allows filtering of the returned list of boost pools based on their chain asset</td>
         <td><code>UncheckedAssetAndChain</code></td>
      </tr>
      <tr>
...
If no `chainAsset` **or** `feeTierBps` is provided, it returns the full list of available boost pools with their respective depths.
```typescript copy
