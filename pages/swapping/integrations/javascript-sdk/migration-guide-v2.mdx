...
<Callout type="info">
  If you plan to implement [DCA](../../swapping-basics.mdx#dollar-cost-average-dca-improving-price), please follow this guide to use these new methods (v2). Otherwise, you can skip it.
</Callout>
...
- [`requestDepositAddress()`](../javascript-sdk/swap-assets/request-deposit-address.mdx)
...
Due to the introduction of DCA, we have added a new method to get quotes for DCA swaps [`getQuoteV2()`](./get-quote/v2.mdx.mdx)
...
The `requestDepositAddress()` [method](../javascript-sdk/swap-assets/request-deposit-address.mdx) argument `DepositAddressRequest` has been updated to accept an additional optional parameter `dcaParams`.
The recommended value for these params is returned in the `getQuoteV2()` method. If you open a deposit channel with these params, the channel will open as a DCA channel and any deposit to this channel will be split into multiple chunks and swapped accordingly.
