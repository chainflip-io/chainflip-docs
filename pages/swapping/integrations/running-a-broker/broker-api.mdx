...
Create an encoded swap with the given parameters. The encoding depends on the chain of the input asset. At the moment, only Bitcoin encoding is supported.
The encoded data is then used to request a swap without opening a channel. See [Bitcoin Vault Swaps](../advanced/vault-swaps/#bitcoin) for more details.
...
Return:
- encoded swap depending on the input asset chain. See [Bitcoin Vault Swaps](../advanced/vault-swaps/#bitcoin) for more details.

Example output:
```json
"Bitcoin": {
  "nulldata_utxo": "0x6a2c01abababababababababababababababababababab0000e80300000000000000000000000000000100020000"
}
```
