...
- `refund_parameters`: [Slippage protection](../../swapping-basics#slippage-protection) for swaps through the channel as JSON object: `{"retry_duration": <number>, "refund_address": <address>, "min_price": <hex_string>, "max_oracle_price_slippage": <number | undefined>}`. Where `retry_duration` is a number of blocks, `refund_address` is an [address](#addresses), `min_price` is a [price](#price), and `max_oracle_price_slippage` is basis points.
...
- `extra_parameters`: JSON object with different data depending on the chain of the input asset.
  - `chain`: The chain of the input asset. Use only one of the following, then provide the corresponding parameters:
    - `Bitcoin`:
      ...
      - `max_oracle_price_slippage`: [Live price protection](../../swapping/swapping-basics#maximum-oracle-price-slippage) in basis points (100th of a percent) as a `u8` number (set to 255 to disable).
