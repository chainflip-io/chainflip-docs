...
- `channel_metadata`: (Optional) [Cross-chain message](../advanced/cross-chain-messaging.mdx) metadata as a JSON object: `{"gas_budget": <amount>, "message":<hex_string>, "cf_parameters": <hex_string>}`. Where `message` and `cf_parameters` are hex encoded strings.
