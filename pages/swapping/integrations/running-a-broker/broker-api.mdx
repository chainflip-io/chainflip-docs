...
Request an incoming transaction to be rejected. Only implemented for bitcoin. This call will always succeed, but the transaction will
...
Parameters:
- `tx_id`: The transaction id of the transaction to be rejected. Currently only bitcoin is supported, the transaction hash has to be specified as shown in
the example below. Also, note that the byte order of the hash is **reverse** to the usual representation in bitcoin block explorers.
...
Returns all deposit channels which have been opened *and are relevant* for `broker_mark_transaction_for_rejection`. Currently only returns bitcoin channels.
...
Subscribes to all runtime events related to rejection of transactions. This establishes a websocket subscription, for testing a utility like `websocat` is useful.

Parameters: None

Return:
- A websocket subscription for transaction screening events. The subscription returns all screening events, i.e. they are **not** filtered by responsible broker. 
  There are currently the following event types:
