...
Mark an incoming transaction to be refunded. Currently only implemented for bitcoin. This rpc method will always succeed, but the transaction will
only be refunded if it hasn't been witnessed yet on chain. For non-boosted deposit-channels this means prior to 6 blocks confirmation, for boosted 
deposit-channels prior to 1 blocks confirmation, i.e. only while the transaction is still in the mempool. Also, a broker can only mark transactions
going into its *own* deposit-channels for rejection.

Parameters:
- `tx_id`: The transaction id of the transaction to be rejected. Currently only bitcoin is supported, the transaction hash has to be specified as show in
the example below. Also, make sure that the byte order of the hash is **reverse** to the usual representation in bitcoin block explorers.

Return:
- Nothing if transaction id was successfully marked for rejection, otherwise error message.
...
Returns all deposit channels which have been opened **and are relevant for `broker_mark_transaction_for_rejection`**. This means that currently only opened bitcoin
deposit channels are returned.

Parameters:
- `query`: Whose deposit channels to take into account. Possible values are `Mine` and `All`.
...
Subscribes to all runtime events related to rejection of transactions. This establishes a websocket subscription, so for manual testing a utility like `websocat` can be used.

Parameters: None

Return:
- A websocket subscription of transaction screening events. There are currently the following event types: `TransactionRejectionRequestReceived`, `TransactionRejectedByBroker` and `TransactionRejectionRequestExpired`.
  The subscription returns all screening events, i.e. they are **not** filtered by responsible broker. 
