...
- **Fully-Delegated**: Legacy-style Affiliate accounts were partially delegated: An Affiliate could request that a broker open channels on its behalf, but was still required to create and register their own account in order to withdraw any earnings. New-style Affiliate accounts are *fully-delegated*, meaning Affiliates can now delegate all of the functionality to the Broker and no longer need to interact with the Chainflip network directly.
...
In the first example, Broker #1 and Broker #2 are two different Chainflip accounts, and opening Swap Deposit Channels on behalf of users, collecting 100bps in fees on every successful swap.
...
<Callout type="info">
A Beneficiary can be either another Broker or an Affiliate.
</Callout>
