...
When submitting a request to open a Swap Deposit Channel a Broker can specify a list of additional *Beneficiaries*, each with their own commission rate. When a swap is executed via a Deposit Channel, the broker and each beneficiary will earn the rates specified when the channel was opened.
