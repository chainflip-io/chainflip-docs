...
Chainflip allows 2 forms of slippage protection, Minimum price, and maximum oracle slippage.
...
The protocol also requires a refund address and a retry duration. 
...
These protections are enforced on the AMM level and do not take deposit fees and broadcast fees into account,
but the Network and broker fees will effect these protections and must be taken into account when setting them.
...
This form of protection is not available to all assets,
opening a deposit channel or encoding vault params with a maximum oracle slippage for an asset that is not supported will return an error.
...
If [slippage protection](#slippage-protection) is enabled, the minimum price requirement is applied to each
individual chunk.
