...
## Trading execution time for better price: DCA
...
If the same chunks reaches its retry limit, it will be refunded along with any remaining chunks in a single refund transaction.
In that case, any amount already successfully executed in prior chunks is still sent to the user specified desitnation address.
