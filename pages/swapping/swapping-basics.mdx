...
<Callout type="info">
  While the protocol allows for dca parameters (number of chunks) to be set by the broker, if the deposited amount is too small - the protocol will adjust the number of chunks to internally match with the minimum chunk size.
  The minimum chunk sizes for assets can be accessed via the RPC method `cf_environment`
</Callout>
