...
Note that all Chainflip swap get scheduled 2 state chain blocks after the deposit that triggers it is witnessed,
so there is about 12 seconds between a swap first appearing in the subscription and it being executed.
...
- `swap_id`: number
- `base_asset`: [Asset](lp-api#assets),
- `quote_asset`: [Asset](lp-api#assets),
- `side`: [Side](lp-api#side),
- `amount`: number,
- `source_asset`: (optional) [Asset](lp-api#assets),
- `source_amount`: (optional) number,
- `execute_at`: number
...
> {"id":1, "jsonrpc":"2.0", "method": "cf_subscribe_scheduled_swaps", "params": ["FLIP", "USDC"]}
```
...
We can see that the `FLIP->USDC` swap will be executed in the next block (85).
We can also see that only the second leg (`USDC->FLIP`) of the `BTC->FLIP` is returned since it is the one that
involves the pool we requested.
Note that while the `amount` field in the first swap is the exact amount that will be executed, the `amount` in
...
Here is what the next notification might look like:
...
Using `swap_id` to identify swaps, we can see that swap 1 has already been executed, but swap 2 appears again as it is still scheduled for the next block.
