...
Users can fund a State Chan account by making a smart contract call to the contract and providing an amount of $FLIP to be locked in the contract. Tokens need to be approved first by the user to be transferred by the contract.
...
After the user has submitted a redemption request to the State Chain, the protocol will submit a redemption to the State Chain Gateway. The contract will store it as a Redemption struct. The redemption is considered to be pending and can be checked with the view function below.
