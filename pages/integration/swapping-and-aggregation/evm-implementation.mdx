...
Chainflip implements smart contracts in EVM compatible chains to achieve all the functionality required for secure asset management, cross-chain messaging and providing a gateway into the State Chain. This page aims to provide a in-depth look at the implementation to make for a smoother integration.

## Swapping via a smart contract call to the Vault

The Vault contract, besides holding assets, acts as an entry point for starting a swap without requiring the creation of a deposit address. See [How Swapping Works](how-swapping-works.md).
...
### Swap and call a receiver with an optional message
...
### Receive the destination asset and a call
