...
1. **Initiating a swap:** This first step can be done through the [Swapping app](https://swap-perseverance.chainflip.io/) (frontend), the [Javascript SDK](javascript-sdk/introduction), or directly using the chainflip CLI client. There are two primary methods to start a Chainflip Swap:
   ...
   - **Direct call to the Vault Smart Contract (EVM chains only):** This method involves calling the `swap()` function in Chainflip Vault contracts. It offers a higher degree of control, making it suitable for advanced users (like DEX Aggregators). See [Execute Swap / Call](javascript-sdk/swap-assets/execute-swap.md) method. For more details on the smart contract implementation see [EVM implementation](EVM-implementation.mdx "mention").
...
<Callout type="info">
  ...
  Explorer](https://blocks-perseverance.chainflip.io/swaps) to see the swap '_in
  progress_'.
</Callout>
