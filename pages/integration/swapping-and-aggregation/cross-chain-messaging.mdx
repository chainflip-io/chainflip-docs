...
- Message should not be longer than 10k bytes.
...
The final `gasLimit` has to be more than the amount of gas the transaction will consume on the destination chain, otherwise the transaction won't be broadcasted. That gas limit is capped to 10M gas. There is multiple factors to take into account when estimating the gas budget:
...
$gasBudget = (userLogicGas + 120k + messageLength * 17) * (baseFee + priorityFee) * overestimationRatio * swapConversion $

An overestimation, `overestimationRatio`, is advised both due to the gas prices and the gas swap being non-deterministic and constantly changing.
...
$gasBudget = nativeTokenNeeded * swapConversion = (3,170,310 / 10^9) * 0.05 = 0.00016 BTC$
...
Chainflip will use TSS to sign over a valid payload for the destination chain and try to broadcast it. If the transaction can't be broadcasted for any reason, the signed payload will be displayed for the user. The user can then sign over the payload and broadcast the transaction. 
