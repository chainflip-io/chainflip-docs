...
Chainflip's Cross-chain Messaging (CCM) capabilities allow developers to not just swap native tokens between chains but also call smart contracts on supported EVM-compatible chains passing an arbitrary message.

Chainflip's unique architecture unlocks a very exciting feature in the cross-chain ecosystem - interoperability between smart contract and non-smart contract blockchains that goes beyond the simple transfer of assets. As described [here](how-swapping-works.md), swaps in Chainflip can be initiated via a smart contract call (see [here](evm-implementation/swap-via-vault.md)) and via requesting deposit addresses (see [here]((javascript-sdk/swap-assets/execute-swap.md))). The latter allows for the initiation of a swap in a non-smart contract blockchain containing a message that allows for execution of the desired logic on the destination chain.
...
For more implementation details around the cross-chain messaging logic see [Vault](evm-implementation/swap-via-vault.md).
...
2. The user requests de bitcoin deposit address as described in [Execute Swap with CCM](javascript-sdk/swap-assets/execute-swap.md). It's worth pointing out that if the source chain is also an EVM-compatible chain, this can be initiated via a smart contract call.
...
4. Chainflip will swap the `gasBudget` amount to the destination chain's native token to pay for gas and the remained of the input amount will be swapped to the destination token.
...
6. The receiver contract will execute the desired logic, which can entail decoding the received messaged from the source chain and acting accordingly. For example, it could then swap USDC into a long tail asset in a Uniswap pool.