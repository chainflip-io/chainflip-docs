...
We distribute our binarries as debian packages through our `apt` repository. To install the packages, you need to add our repo to your `apt` sources list.
...
If you are planning to run a validator using a contaier orchestration system like Kubernetes you can use our pre-built Docker on [Docker Hub](https://hub.docker.com/u/chainfliplabs).
...
To start the node, you would need the following optionts passted to the `chainflip-node` binary:

```bash copy
chainflip-node \
  --chain=/path/to/chainflip/dir/perseverance.chainspec.json \
...
If you have installed binaries through APT, the chainspec is alreay included in the `chainflip-node` package. However, if you need the chainspec for Kubernetes deployments or other docker orchestration systems, you can download it by running:
