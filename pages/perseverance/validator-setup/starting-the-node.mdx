...
```bash copy
systemctl status chainflip-engine0.10
```

Check the engine logs:

```bash copy
journalctl -f -u chainflip-engine0.10.service
```
...
```bash copy
sudo systemctl enable chainflip-node0.10
```

```bash copy
sudo systemctl enable chainflip-engine.10
```

<Callout type="info">
  Anytime you make changes to your config file, don't forget to run: `systemctl
  restart chainflip-engine0.10`
</Callout>
