...
You must ensure that the public EVM Chain address generated above has at least 0.1 of each native token on the corresponding chain (ETH, ArbETH, ... etc). This requirement is subject to change based on transaction fees but for now should be sufficient.
