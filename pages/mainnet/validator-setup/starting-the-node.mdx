...
```bash copy
sudo systemctl start chainflip-engine1.2
```

To check on the service, we use `status`.

```bash copy
systemctl status chainflip-engine1.2
```

Check the engine logs:

```bash copy
journalctl -f -u chainflip-engine1.2.service
```
